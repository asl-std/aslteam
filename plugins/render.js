    ...
    renderMenu()
    {
        function HTML_MENU(data)
        {
            return `
					...
					<li class="active stick"><a>${data.I2}</a></li>
...
class renderReviews
{
    ...
    renderContent()
    {
        ...
        function HTML_COMMENTS(data)
        {
			...
			let VK = "", Spigot = "", Polymart = "", MCMarket = "", GitHub = "", Description = "";
			if(data.description){Description = `<div class="desc">${data.description}</div>`}
			...
			let freewrap = "paid-wrap", free = "paid", type = "yespaid";
			if(data.free){freewrap = "free-wrap"; free = "free"; type = "nopaid";}
	...
	renderSort()
    {
		document.querySelector(".dropbtn").textContent = this.data.sort.category;
		document.querySelector("#all").textContent = this.data.sort.all;
        document.querySelector("#free").textContent = this.data.sort.free;
		document.querySelector("#paid").textContent = this.data.sort.paid;
    }
...
document.querySelector('#all').addEventListener("click", () => {
	let e = document.getElementsByClassName("nopaid");
	for (i = 0; i < e.length; i++) {let el = e[i];el.classList.remove('hide')};

	let t = document.getElementsByClassName("yespaid");
	for (i = 0; i < t.length; i++) {let el = t[i];el.classList.remove('hide')};
});
document.querySelector('#free').addEventListener("click", () => {
	...
	let t = document.getElementsByClassName("yespaid");
	for (i = 0; i < t.length; i++) {let el = t[i];el.classList.add('hide')};
});
document.querySelector('#paid').addEventListener("click", () => {
	let e = document.getElementsByClassName("nopaid");
	for (i = 0; i < e.length; i++) {let el = e[i];el.classList.add('hide')};
