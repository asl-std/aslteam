const DATABASE = {
    "ru": {
        "MENU": ["Наши услуги", "Наши работы", "О нас", "Отзывы"],
        "SERVICE_TEAM": {
            "serv_01": {
                "parametrs": {
                ...
                "constet": {
                    ...
                    "description": "Данная услуга к сожалению более недоступна :)",
                    "params": {
                        "opt1": {}
                    }
                }
            ...
            "serv_02": {
                ...
                "constet": {
                    ...
                    "description": "Наши умелые разработчики помогут вам решить, как же развлекать игроков, а так же поможем решить технические проблемы, если вдруг вы забыли поставить Essentials!<br>Мы занимаемся разработкой плагинов уже более 5 лет, и мы будем рады помочь вам в создании особого контента на вашем сервере.",
                    "params": {
                        "opt1": {
                            "imgSrc": "point.png",
        ...
        "WORKS_TEAM": {
            "work_01": {
                "img": "point.png",
                "title": "Огромный MMO-RPG Сервер",
                "text": "Огромный MMO-RPG Сервер созданный под впечатленниями перед World Of Warcraft. Идея сервера развивалась много лет, набрав неплохой ЛОР, большое количество геймплейных фич. Сервер подходит всем игрокам, когда либо встречавшимся с жанром RPG, как локальным так и многопользовательским.",
                "link": "https://discord.gg/4NVRjHrcxM"
            },
        },
        "ABOUT_DESCRIPTION": "Мы относительно молодая студия разработчиков, интересующаяся всем и вся в IT индустрии, остановились в сфере Game-Dev, основной состав состоит из 5 человек, всего нас 11 человек. За время работы студии было проделано немало работы в её границах, мы набрались опыта и теперь готовы вас удивлять.",
        "TEAM": {
            "pers_01": {
            ...
            "pers_04": {
                "nikname": "Rai_Fox",
                "description": "Программист JAVA/C++",
                "positions": null,
                "links": null,
                "img": "pers_04.png"
            },
            "pers_05": {
                "nikname": "ZiM",
                "description": "Менеджер собщества",
...
// lang
const TAG_MENU = document.querySelector(".menu__container").children;
const TAG_TITLE = ["seriec", "work", "aboutus", "review"];
const TAG_ABOUT_DESCRIPTION = document.querySelector(".aboutus__description");
const TAG_FOOTER = {
    "left": document.querySelector(".footer__license"),
    "right": document.querySelector(".footer__socNetWork__item"),
};
...
const DATA_LANG = {
    ...
    "set": document.querySelector("#lang--set"),
}
    ...
    openService(item)
    {
        ...
        POP_UP.innerHTML = `
            <div class="service__window">
                <h4 class="service__window__tile">${setLang().SERVICE_TEAM[item].constet.tile}</h4>
                <p class="service__window__description description">
                    ${setLang().SERVICE_TEAM[item].constet.description}
                </p>
                <div class="service__options container">
                    ${this.defineOption(setLang().SERVICE_TEAM[item].constet.params)}
                </div>
...
class Person
{
    ...
    openInfo(pres_id)
    {
        ...
        {
            let arr = [];
            if(setLang().TEAM[pres_id].positions != null)
            {
                for(let i = 0; i < (setLang().TEAM[pres_id].positions.length); i++)
                {
                    arr += `<li class="person__position__item">${setLang().TEAM[pres_id].positions[i]}</li>`;
                }
            }
            return arr;
        }
        ...
        {
            let linksSocNetwork = setLang().TEAM[pres_id].links;
            let arr = [];
        ...
        <div class="person__top mod--grad">
            <img class="person__top__img" src="images/team/${setLang().TEAM[pres_id].img}" alt="Фото члена команды">
            <h3 class="person__top__tile">${setLang().TEAM[pres_id].nikname}</h3>
        </div>
